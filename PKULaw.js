{
	...
	"lastUpdated": "2025-04-10 03:56:48"
}
...
async function scrape(doc, url = doc.location.href) {
	...
	switch (newItem.itemType) {
		...
		case 'report':
			...
			data(['制定机关', '发布部门', 'IssuingAuthority'], true).querySelectorAll('a:first-child').forEach((element) => {
...
function getLabeledData(rows, labelGetter, dataGetter, defaultElm) {
	...
	Z.debug(Object.entries(labeledElm).map(([key, val]) => [key, val.textContent]));
